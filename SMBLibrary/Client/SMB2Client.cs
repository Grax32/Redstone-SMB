...
namespace SMBLibrary.Client
{
    public class SMB2Client : ISMBClient
    {
        ...
        private List<SMB2Command> _incomingQueue = new List<SMB2Command>();
        private EventWaitHandle _incomingQueueEventHandle = new EventWaitHandle(false, EventResetMode.AutoReset);

        private SessionPacket _sessionResponsePacket;
        private EventWaitHandle _sessionResponseEventHandle = new EventWaitHandle(false, EventResetMode.AutoReset);
        ...
        private bool ConnectSocket(IPAddress serverAddress, int port)
        {
            ...
            _clientSocket.BeginReceive(buffer.Buffer, buffer.WriteOffset, buffer.AvailableLength, SocketFlags.None, new AsyncCallback(OnClientSocketReceive), state);
            return true;
        }
        ...
        public ISMBFileStore TreeConnect(string shareName, out NTStatus status)
        {
            ...
            IPAddress serverIPAddress = ((IPEndPoint)_clientSocket.RemoteEndPoint).Address;
